...
with st.form("translation_form"):
    ...
    # --- NUEVO ORDEN DE ENTRADA DE TEXTO ---
    st.markdown("### Texto a traducir:")
    
    # El capturador de voz queda posicionado en el medio
    audio_file = st.audio_input("Dictar por voz (opcional):")
    
    # Si graba un audio fuera del submit, procesamos la transcripción de inmediato
    if audio_file:
    ...
    # El campo de texto definitivo abajo de la entrada de voz
    ...
    if not user_query:
        ...
        with st.spinner("Traduciendo y generando audio..."):
            try:
                ...
                # Botón de Copiar al Portapapeles mediante JS inyectado en un botón invisible
                # Usamos st.components.v1.html para inyectar la lógica del portapapeles de forma segura
                js_button = f"""
                <script>
                function copyText() {{
                ...
                # El reproductor de Streamlit ya cuenta de forma nativa con los 3 puntitos 
                # a la derecha que permiten "Descargar" el archivo de audio directamente.
                st.audio(audio_path, format="audio/mp3")
                
                # Botón de descarga explícito adicional (por si querés un acceso más directo)
