...
# Colores para el botón HTML de copiado
btn_copy_bg = "#31333F" if st.session_state.theme_mode == 'dark' else "#EEEEEE"
btn_copy_text = "#FFFFFF" if st.session_state.theme_mode == 'dark' else "#121212"
btn_copy_border = "#4A4B57" if st.session_state.theme_mode == 'dark' else "#CCCCCC"

# --- INYECCIÓN DE ESTILOS LIMPIOS ---
st.markdown(f"""
    <style>
        .block-container {{
        ...
        /* Evita conflictos y fuerza que los radios tengan margen uniforme */
        div[data-testid="stRadio"] div[role="radiogroup"] {{
            gap: 15px;
        }}
        
        /* Forzar color azul en el botón primario (Traducir) */
...
# Selectores de Idioma (Alineados abajo con vertical_alignment)
col_de, col_inter, col_a = st.columns([4.5, 1, 4.5], vertical_alignment="bottom")
with col_de:
    st.markdown('<p class="section-label" style="margin-top:0px;">De:</p>', unsafe_allow_html=True)
...
with col_a:
    st.markdown('<p class="section-label" style="margin-top:0px;">A:</p>', unsafe_allow_html=True)
...
# Entrada de Texto y Grabador de voz (Alineados abajo con vertical_alignment)
st.markdown('<p class="section-label">Texto a traducir / Dictar:</p>', unsafe_allow_html=True)
col_txt, col_mic = st.columns([8.2, 1.8], vertical_alignment="bottom")
...
st.write("")
col_borrar, col_traducir = st.columns(2, vertical_alignment="bottom")
...
if st.session_state.translated_text:
    ...
    col_dl, col_cp = st.columns(2, vertical_alignment="bottom")
    with col_dl:
    ...
    with col_cp:
        ...
        <button onclick="copyText()" style="background-color: {btn_copy_bg}; color: {btn_copy_text}; border: 1px solid {btn_copy_border}; padding: 6px 16px; border-radius: 6px; cursor: pointer; width:100%; height:38px; font-family:sans-serif; font-size:14px;">📋 Copiar Texto</button>
        """
